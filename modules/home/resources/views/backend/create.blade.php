...
@section('breadcrumb')
    ...
    <a class="breadcrumb-item active" href="{{ route('home.create') }}">@lang('global.add')</a>
@endsection
...
@section('content')
    <div class="br-section-wrapper">
        ...
        {{-- Bg content --}}
        <form action="{{ route('home.store') }}" method="post" enctype="multipart/form-data" id="frm_create_home">
            @csrf
...
@section('script')
    <script>
        $('#frm_create_home').on('submit', function (event) {
            event.preventDefault();

            var form = $('#frm_create_home');
            ...
            createHome(form.serialize());
        });

        $('#frm_create_home').validate({
            errorElement: "span",
            rules: {
                name: {
                    required: true
