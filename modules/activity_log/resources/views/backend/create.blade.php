...
@section('breadcrumb')
    ...
    <a class="breadcrumb-item active" href="{{ route('activity_log.create') }}">@lang('global.add')</a>
@endsection
...
@section('content')
    <div class="br-section-wrapper">
        ...
        {{-- Bg content --}}
        <form action="{{ route('activity_log.store') }}" method="post" enctype="multipart/form-data" id="frm_create_activity_log">
            @csrf
...
@section('script')
    <script>
        $('#frm_create_activity_log').on('submit', function (event) {
            event.preventDefault();

            var form = $('#frm_create_activity_log');
            ...
            createActivityLog(form.serialize());
        });

        $('#frm_create_activity_log').validate({
            errorElement: "span",
            rules: {
                name: {
                    required: true
        ...
        function createActivityLog(data) {
            $.ajax({
                url: app_url + 'admin/activity_log',
