...
@section('breadcrumb')
    <a class="breadcrumb-item active" href="{{ route('lienquanrank.index') }}">{{ $display_name }}</a>
...
@section('content')
    <div class="br-section-wrapper">
        ...
        <div class="col-sm-2 col-md-2 pd-0">
            <button class="btn btn-info btn-block mg-b-20" onclick="window.location='{{ route('lienquanrank.create') }}'">
        ...
        <div class="rounded table-responsive">
            <table class="table table-bordered mg-b-0" id="lienquanrank_table">
                <thead>
                <tr>
...
@section('script')
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#lienquanrank_table').DataTable({
                autoWidth: true,
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: app_url + 'admin/lienquanrank/get_list_lienquanrank',
                    type: 'post'
                },
                searching: true,
                columns: [
            ...
            $('#lienquanrank_table').on('click', '.btn-edit', function () {
                window.location.href = app_url + 'admin/lienquanrank/' + $(this).data('id') + '/edit';
            });

            $('#lienquanrank_table').on('click', '.btn-delete', function (event) {
                ...
                swal({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: app_url + 'admin/lienquanrank/' + $(this).data('id'),
                            type: 'DELETE',
                            dataType: "JSON",
                            data: {
                                id: $(this).data('id')
                            ...
                            success: function (res)
                            {
                                if (!res.err) {
                                    ...
                                    $('#lienquanrank_table').DataTable().ajax.reload();
                                } else {
